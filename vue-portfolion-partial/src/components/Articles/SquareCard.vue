<template>
    <router-link  :to="'/articles/' + article._id">
        <div class="article" :class="[ (isLarge) ? 'article-large' : 'article-full-pic', color ]">
            <div class="article-img-container" v-if="showImage">
                <img class="hero-img" v-bind:src="baseUrl + article.hero.url" alt="">
            </div>
            <div class="article-hover bottom-left px-8 py-4">
                <div class="mb-3">
...
export default {
    props: ['article', 'color', 'showImage', 'showText', 'isLarge'],
    data() {
        return {
